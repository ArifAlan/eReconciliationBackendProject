...
namespace Business.Concrete
{
    public class MailTemplateManager : IMailTemplateService
    {
        ...
        {
            return new SuccessDataResult<MailTemplate>(_mailTemplateDal.Get(p => p.Id == id));
        }

        public IDataResult<MailTemplate> GetByTemplateName(string name, int companyId)
        {
            return new SuccessDataResult<MailTemplate>(_mailTemplateDal.Get(p => p.CompanyId == companyId && p.Type==name));
        }
        ...
        IDataResult<List<MailTemplate>> IMailTemplateService.GetAll(int companyId)
        {
            return new SuccessDataResult<List<MailTemplate>>(_mailTemplateDal.GetList(p => p.CompanyId == companyId));
        }
    }
}
